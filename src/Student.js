...
/**
 * src/Student.js
 * Class Student dengan komentar berpenomoran untuk melacak alur.
 */

class Student {
  /**
   * [12] Constructor dipanggil ketika `new Student(id,name,class)` di index.js
   * @param {string} id
   * @param {string} name
   * @param {string} className
   * @param {object} grades (optional)
   */
  constructor(id, name, className, grades = {}) {
    // [12] Validasi input awal
    if (!id && id !== 0) throw new Error('ID siswa wajib diisi.');
    ...
    this._className = String(className).trim();
    this._grades = {};

    // [12] Masukkan initial grades bila ada dan valid
    for (const [subj, score] of Object.entries(grades)) {
      const n = Number(score);
      if (!Number.isFinite(n) || n < 0 || n > 100) continue;
      this._grades[String(subj)] = n;
    }
  }
  ...
  set className(newClass) {
    if (!newClass || String(newClass).trim() === '')
      throw new Error('Kelas tidak boleh kosong.');
    this._className = String(newClass).trim();
  }

  /**
   * [42] addGrade(subject, score) dipanggil dari index.js langkah [42]
  ...
  addGrade(subject, score) {
    ...
    this._grades[String(subject).trim()] = s;
    return true;
  }

  /**
   * [44] getAverage() akan dipanggil untuk menampilkan rata-rata setelah tambah nilai
   * @returns {number} rata-rata (2 desimal)
   */
  getAverage() {
    const keys = Object.keys(this._grades);
    if (keys.length === 0) return 0; // jika belum ada nilai
    const total = keys.reduce((acc, k) => acc + (this._grades[k] || 0), 0);
    const avg = total / keys.length;
    return Math.round(avg * 100) / 100;
  }

  /**
   * [44] getGradeStatus() menentukan Lulus/Tidak Lulus
   * Kriteria: rata-rata >= 75 => Lulus
   */
  getGradeStatus() {
    const avg = this.getAverage();
    if (avg === 0 && Object.keys(this._grades).length === 0)
      return 'Belum Ada Nilai';
    return avg >= 75 ? 'Lulus' : 'Tidak Lulus';
  }

  /**
   * [25]/[29]/[39] displayInfo() digunakan di beberapa langkah index.js
   * Mengembalikan string yang merangkum data siswa
   */
  displayInfo() {
    const lines = [];
    lines.push(`ID: ${this._id}`);
    lines.push(`Nama: ${this._name}`);
    lines.push(`Kelas: ${this._className}`);
    lines.push('Mata Pelajaran:');
    const gradeKeys = Object.keys(this._grades);
    if (gradeKeys.length === 0) {
      lines.push('  - (Belum ada nilai)');
    } else {
      for (const subj of gradeKeys) {
        lines.push(`  ${subj}: ${this._grades[subj]}`);
      }
    }
    lines.push(`Rata-rata: ${this.getAverage()}`);
    lines.push(`Status: ${this.getGradeStatus()}`);
    return lines.join('\n');
  }

  /**
   * toJSON & fromObject => membantu persistence (dipanggil oleh StudentManager [43])
   */
  toJSON() {
    return {
      id: this._id,
  ...
  static fromObject(obj) {
    if (!obj) throw new Error('Invalid object untuk Student.fromObject');
    return new Student(obj.id, obj.name, obj.className, obj.grades || {});
  }
}
